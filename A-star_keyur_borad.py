...
def cost_to_go(x1,y1,x2,y2):
    hh=int(((x2-x1)**2+(y2-y1)**2)**(1/2))
    return hh
...
start_node=(cost_to_come+cost_to_go(init_x,init_y,final_x,final_y),node_index,parent_node_index,(init_x,init_y),cost_to_come,cost_to_go(init_x,init_y,final_x,final_y))
    ...
    # If not following code is executed
        ...
        for action in listt:
            ...
            if action_output[0]<=1194 and action_output[0]>=5 and action_output[1]<=494 and action_output[1]>=5:
                ...
                #print(new_node[0],new_node[1])
                action_c2c=action_output[2]
                # heuristic=int(cost_to_go(new_node[0],new_node[1],final_x,final_y))
                heur = cost_to_go(new_node[0],new_node[1],final_x,final_y)
                    ...
                    if {new_node}.issubset(Obstacle_Space)==False:
                            ...
                            for i in range(len(open_nodes)):
                                    if open_nodes[i][3]==new_node:
                                        ...
                                        if open_nodes[i][0]>(cost_to_come+action_c2c+heur):
                                            open_nodes[i]=(cost_to_come+action_c2c+heur,open_nodes[i][1],current_node[1],open_nodes[i][3],cost_to_come+action_c2c,heur)
                                            break
                                        else:
                                            break
                            ...
                            if open_node_flag==False:
                                node_index+=1
                                hq.heappush(open_nodes,(cost_to_come+action_c2c+heur,node_index,current_node[1],new_node,cost_to_come+action_c2c,heur))
